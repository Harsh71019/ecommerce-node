  ...
  requestLogger(req, res, async () => {
    ...
    if (user) {
      res.status(201).json({
        ...
        email: user.email,
        mobile: (user as UserType).mobile,
      });
    } else {
  ...
  requestLogger(req, res, async () => {
    try {
      ...
      if (user) {
        ...
        (user as UserType).mobile =
          req.body.mobile || (user as UserType).mobile;
        ...
        res.json({
          ...
          mobile: (updatedUser as UserType).mobile,
...
const resetPassword = asyncHandler(async (req, res) => {
  try {
    ...
    // @ts-expect-error TS(2304): Cannot find name 'jwt'.
    ...
    (user as UserType).resetPasswordToken = undefined;
    await user.save();
