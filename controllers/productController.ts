...
const getProducts = asyncHandler(async (req, res) => {
  ...
  if (req.query.category) {
    (filters as any).category = req.query.category;
  }
  if (req.query.brand) {
    (filters as any).brand = req.query.brand;
  }
  // Filter by price range
  if (req.query.minPrice || req.query.maxPrice) {
    (filters as any).price = {};
    if (req.query.minPrice) {
      (filters as any).price.$gte = parseFloat(req.query.minPrice as string);
    }
    if (req.query.maxPrice) {
      (filters as any).price.$lte = parseFloat(req.query.maxPrice as string);
    }
  }
  // Filter by discount
  if (req.query.minDiscount) {
    (filters as any).discount = {
      $gte: parseFloat(req.query.minDiscount as string),
    };
  }
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    (product as any).isCouponEligible =
      isCouponEligible || (product as any).isCouponEligible;
    ...
    try {
      if (images && Array.isArray(images) && images.length > 0) {
        ...
        const imagesToDelete = oldImages.filter(
          (oldImage: any) =>
            // @ts-expect-error TS(7006): Parameter 'oldImage' implicitly has an 'any' type.
            (oldImage) =>
              !images.some(
        ...
        for (const imageToDelete of imagesToDelete) {
          await (cloudinary as any).uploader.destroy(imageToDelete.public_id);
        }
    ...
    try {
      if (newImages && Array.isArray(newImages) && newImages.length > 0) {
        for (const newImage of newImages) {
          const result = await (cloudinary as any).uploader.upload(newImage);
          const newImageData = {
...
const updateProductReview = asyncHandler(async (req, res) => {
  ...
  if (product) {
    const reviewIndex = product.comments.findIndex(
      (comment: any) => comment._id.toString() === req.params.reviewId,
    );
